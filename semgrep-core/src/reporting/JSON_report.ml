...
let metavar_string_of_any any =
  (* TODO: metavar_string_of_any is used in get_propagated_value
     ...
     Handle such cases *)
  any |> V.ii_of_any |> string_of_toks
...
let tokens_to_loc_and_content toks =
  ...
  in
  let text = string_of_toks toks in
  Some (location, text)

let rec taint_call_trace_to_taint_source = function
  | Toks toks ->
      let* location, abstract_content = tokens_to_loc_and_content toks in
      Some ({ Out.location; abstract_content } : Out.taint_source)
  | Call { call_trace; _ } -> taint_call_trace_to_taint_source call_trace

let token_to_intermediate_var token =
  let* location, abstract_content = tokens_to_loc_and_content [ token ] in
  Some ({ Out.location; abstract_content } : Out.intermediate_var)
